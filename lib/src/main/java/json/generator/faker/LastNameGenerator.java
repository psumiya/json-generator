...
public record LastNameGenerator(FakerStringSupplier fakerStringSupplier) implements FieldGenerator {
    ...
    public JsonNode generate(FieldConfiguration configuration, JsonNode seedValue) {
        return fakerStringSupplier.apply(() -> fakerStringSupplier.faker().name().lastName());
    }
